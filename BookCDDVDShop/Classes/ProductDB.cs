...
namespace BookCDDVDShop.Classes
{
    class ProductDB
    {
        string dbProductType = "";  // Type of record found in database
        string dbStringProduct = ""; 
        string fieldsFound = "";

        // Connection string for ProductDB (type: Microsoft Access) in the Resources folder
        string strConnection = "provider=Microsoft.ACE.OLEDB.12.0;" + "Data Source=../ProductDB-2.accdb";

        // *********** INSERTION METHODS **********
        //Inserts into Product table with parameters UPC, Price, Title, Quantity, and productType
        public bool InsertProduct(int UPC, decimal price, string title, int quantity, string productType)
        {
            ...
            OleDbCommand myCommand = new OleDbCommand(strInsertProduct, myConnection); //Create a command instance
            OleDbDataReader myDataReader;

            try //Try to insert into database
            {
                myConnection.Open(); //Open database connection
                myDataReader = myCommand.ExecuteReader(); //Run command on database
                myDataReader.Close(); //Close database connection
                return true; //Insert was successful
            }
            catch (OleDbException ex) //Catch if try fails
            {
                MessageBox.Show("There was an Insert Product error: " + ex.Message,
                    "Product Insert Failed", MessageBoxButtons.OK); //Display error message
                return false; //Insert was unsuccessful
            }
        ...
        //Insert into Book table with parameters UPC and ISBN, Author, Pages
        public bool InsertBook(int UPC, int ISBN, string author, int pages)
        {
            ...
            OleDbConnection myConnection = new OleDbConnection(strConnection); //Create a connection instance 
            OleDbCommand myCommand = new OleDbCommand(strInsertBook, myConnection); //Create a command instance
            OleDbDataReader myDataReader;
            ...
            catch (OleDbException ex) //Catch if try fails
            {
                MessageBox.Show("There was an Insert Book error: " + ex.Message,
                     "Book Insert Failed", MessageBoxButtons.OK); //Display error message
                return false;  //Insert was unsuccessful
            }
            finally //Executes after try/catch
            {
        ...
        //Inserts into BookCIS table with parameters UPC and CISArea
        public bool InsertBookCIS(int UPC, string CISArea)
        {
            ...
            OleDbConnection myConnection = new OleDbConnection(strConnection); //Create a connection instance
            OleDbCommand myCommand = new OleDbCommand(strInsertBookCIS, myConnection); //Create a command instance
            OleDbDataReader myDataReader;
            ...
            catch (OleDbException ex) //Catch if try fails
            {
                MessageBox.Show("There was an Insert BookCIS error: " + ex.Message,
                    "BookCIS Insert Failed", MessageBoxButtons.OK); //Display error message
                return false; //Insert was unsuccessful
            }
            finally //Executes after try/catch
            {
        ...
        //Inserts into DVD table with parameters UPC and Lead Actor, Release Date and Run Time
        public bool InsertDVD(int UPC, string lead, DateTime relDate, int runTime)
        {
            ...
            OleDbConnection myConnection = new OleDbConnection(strConnection); //Create a connection instance
            OleDbCommand myCommand = new OleDbCommand(strInsertDVD, myConnection); //Create a command instance
            OleDbDataReader myDataReader;
            ...
            catch (OleDbException ex) //Catch if try fails
            {
                MessageBox.Show("There was an Insert DVD error: " + ex.Message,
                   "DVD Insert Failed", MessageBoxButtons.OK); //Display error message
                return false; //Insert was unsuccessful
            }
            finally //Executes after try/catch
            {
        ...
        //Inserts into CDClassical table with parameters ProductUPC and CDClassical Label and CDClassical Artists
        public bool InsertCDClassical(int UPC, string label, string artists)
        {
            ...
            OleDbConnection myConnection = new OleDbConnection(strConnection); //Create a connection instance
            OleDbCommand myCommand = new OleDbCommand(strInsertCDClassical, myConnection); //Create a command instance
            OleDbDataReader myDataReader;
            ...
            catch (OleDbException ex) //Catch if try fails
            {
                MessageBox.Show("There was an Insert CDCLassical error: " + ex.Message,
                    "CDClassical Insert Failed", MessageBoxButtons.OK); //Display error message
                return false; //Insert was unsuccessful
            }
            finally //Executes after try/catch
            {
        ...
        //Inserts into CD Chamber Music table with parameters ProductUPC and InstrumentList
        public bool InsertCDChamber(int UPC, string instrumentList)
        {
            ...
            OleDbConnection myConnection = new OleDbConnection(strConnection); //Create a connection instance
            OleDbCommand myCommand = new OleDbCommand(strInsertCDChamber, myConnection); //Create a command instance
            OleDbDataReader myDataReader;
            ...
            catch (OleDbException ex) //Catch if try fails
            {
                MessageBox.Show("There was an Insert CDChamber error: " + ex.Message,
                    "CDChamber Insert Failed", MessageBoxButtons.OK); //Display error message
                return false; //Insert was unsuccessful
            }
            finally //Executes after try/catch
            {
        ...
        //Inserts into CDOrchestra table with parameters ProductUPC and Conductor
        public bool InsertCDOrchestra(int UPC, string Conductor)
        {
            ...
            OleDbConnection myConnection = new OleDbConnection(strConnection); //Create a connection instance
            OleDbCommand myCommand = new OleDbCommand(strInsertCDOrchestra, myConnection); //Create a command instance
            OleDbDataReader myDataReader;
            ...
            catch (OleDbException ex) //Catch if try fails
            {
                MessageBox.Show("There was an Insert CDOrchestra error: " + ex.Message,
                     "CDOrchestral Insert Failed", MessageBoxButtons.OK); //Display error message
                return false; //Insert was unsuccessful
            }
            finally //Executes after try/catch
            {
        ...
        // Queries/Selects records from all tables that match integer parameter ProductUPC
        // Returns a reference to the retrieved record
        public OleDbDataReader SelectProduct(int ProductUPC, out bool OKFlag)
        {
            // CURRENTLY NOT USED
            //Create sql statement
            string strSelectProduct = 
                "SELECT * FROM Product, Book, DVD, CDClassical, BookCIS, CDOrchestra, CDChamber " +
                " WHERE Product.fldUPC = " + ProductUPC + ", Book.fldUPC = " + ProductUPC +
                ", BookCISUPC.fldUPC = " + ProductUPC + ", DVD.fldUPC = " + ProductUPC + 
                ", CDClassicalUPC.fldUPC = " + ProductUPC + ", CDChamberUPC.fldUPC = " + ProductUPC + 
            ...
            try  //Try to select/query from database
            {
                myConnection.Open(); //Open database connection
            ...
            catch (OleDbException ex)
            {
                MessageBox.Show("There was an Select Product error: " + ex.Message,
                     "Product Select Failed", MessageBoxButtons.OK); //Display error message
                myConnection.Close(); //Close database connection
                OKFlag = false; //Select was unsuccessful
                myDataReader = null;
            }
            return myDataReader;
        }  // end SelectProduct
            ...
            out bool OKFlag, out string fieldsOut)
        {
            ...
            catch (OleDbException ex)
            {
                MessageBox.Show("There was a Select from Product error: " + ex.Message,
                    "Select from Product Failed", MessageBoxButtons.OK); //Display error message
            ...
            out Boolean OKFlag, out string fieldsFound)
        {
            ...
            catch (OleDbException ex)
            {
                MessageBox.Show("There was a Select Product from Book error: " + ex.Message,
                     "Select Book Failed", MessageBoxButtons.OK); //Display error message
            ...
            MessageBox.Show("Book Found ..." + Environment.NewLine
                + fieldsFound, "Show Found Product", MessageBoxButtons.OK); //Display found message
            return myDataReader;
...
dbStringProduct = dbStringProduct + myDataReader["fldPrice"].ToString() + Environment.NewLine;
            ...
            catch (OleDbException ex)
            {
                MessageBox.Show("There was a Select Product from CDClassical error: " + ex.Message,
                    "Select CDClassical Failed", MessageBoxButtons.OK); //Display error message
                myConnection.Close(); //Close database connection
                myDataReader = null;
                OKFlag = false; //Select was unsuccessful
            }
            ...
            MessageBox.Show("CDClassical Found ..." + Environment.NewLine
                + fieldsFound, "Show Found CDCLassical", MessageBoxButtons.OK); //Display found message
            return myDataReader;
            ...
            out bool OKFlag, out string fieldsFound)
        {
            ...
            catch (OleDbException ex)
            {
                MessageBox.Show("There was a Select Product from CDChamber error: " + ex.Message,
                    "Select CDChamber Failed", MessageBoxButtons.OK); //Display error message
                myConnection.Close();
                OKFlag = false; //Select was unsuccessful
                myDataReader = null;
            }
            ...
            MessageBox.Show("CDChamber Found ..." + Environment.NewLine
                + fieldsFound, "Show Found CDChamber", MessageBoxButtons.OK); //Display found message
            return myDataReader;
